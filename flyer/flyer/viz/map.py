...
class MapApp:
    ...
    def __init__(self, directory : str, ip : str, port : int) -> None:
        dire = directory
        self.ip = ip
        self.port = port
        self.app_ = Flask(__name__, 
                          template_folder=os.path.join(dire,'viz','templates'),
                          static_folder=os.path.join(dire,'viz','static'))
    ...
    def run(self) -> None:
        self.app_.run(debug=False, host=self.ip, port=self.port)


class VizNode(Node):

    def __init__(self) -> None:
        super(VizNode, self).__init__('glider_viz')

        self.declare_parameter("directory", "./")
        dire = self.get_parameter("directory").value
